...
def p_loss(k, N, eta):
    """Probability of losing k photons out of N with tranmissivity eta."""
    if k < 0 or k > N:
        return 0
...
def p_conditional(m, N, f, lambd, eta):
    """Conditional probability of having sent bit f (0 or 1) given a photon-count m."""
    u_bound = 1 + int(lambd + 5 * np.sqrt(lambd))
    suma_1 = sum(
...
def bf_optimizer(N, eta, gamma, lambd):
    """Brute-force optimization: finds best threshold l minimizing chi."""
    best_cost = 1e-5
    opt_l = 0
    u_bound = 1 + int(lambd + 5 * np.sqrt(lambd))
    for l in range(N + 1 + u_bound):
        cost_aux = chi(N, eta, gamma, lambd, l)
