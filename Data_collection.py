...
def var_theta(x, y, z, eta, gamma, lambd):
    if z >= x:
        output = f_coherence(x, gamma) * np.sqrt(p_loss(2*x+y-z, 2*x, eta) * p_loss(2*x+y-z, x, eta) * p_dark(y,lambd) * p_dark(y,lambd))
    else:
        output = f_coherence(z-y, gamma) * np.sqrt(p_loss(2*x+y-z, 2*x, eta) * p_loss(x, x, eta) * p_dark(y,lambd) * p_dark(y,lambd))
    return 1/2 * output
